		...
		obs_property_t prop)
{
	...
	frmsize.index = 0;
	char *buffer = bzalloc(20 * sizeof(char));

	obs_property_list_clear(prop);

	while (ioctl(dev, VIDIOC_ENUM_FRAMESIZES, &frmsize) == 0) {
		snprintf(buffer, 20, "%dx%d", frmsize.discrete.width,
				frmsize.discrete.height);
		obs_property_list_add_int(prop, buffer,
		...
		uint_fast32_t width, uint_fast32_t height, obs_property_t prop)
{
	...
	while (ioctl(dev, VIDIOC_ENUM_FRAMEINTERVALS, &frmival) == 0) {
		...
		snprintf(buffer, 20, "%.2f", fps);
		obs_property_list_add_int(prop, buffer, pack);
		frmival.index++;
	}
	bfree(buffer);
}
